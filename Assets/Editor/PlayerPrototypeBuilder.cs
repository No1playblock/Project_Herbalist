...
namespace Herbalist.Editor
{
    ...
    {
        private const string Root = "Assets/PlayerPrototype";
        public static string Build()
        {
            if (Application.isPlaying) throw new InvalidOperationException("Build in edit mode.");
            if (AssetDatabase.LoadAssetAtPath<GameObject>(Root + "/Player.prefab") != null)
                throw new InvalidOperationException("Prototype already exists; edit the existing assets.");
            if (!AssetDatabase.IsValidFolder(Root)) AssetDatabase.CreateFolder("Assets", "PlayerPrototype");
            var scene = EditorSceneManager.NewScene(NewSceneSetup.EmptyScene, NewSceneMode.Additive);
            SceneManager.SetActiveScene(scene);

            var tuning = ScriptableObject.CreateInstance<PlayerTuning>();
            AssetDatabase.CreateAsset(tuning, Root + "/PlayerTuning.asset");
            ...
            System.IO.File.WriteAllText(Root + "/PlayerControls.inputactions", actions.ToJson());
            UnityEngine.Object.DestroyImmediate(actions);
            AssetDatabase.ImportAsset(Root + "/PlayerControls.inputactions");
            actions = AssetDatabase.LoadAssetAtPath<InputActionAsset>(Root + "/PlayerControls.inputactions");
            ...
            EditorSceneManager.SaveScene(scene, Root + "/PlayerMovementPrototype.unity");
        ...
        private static InputActionReference MakeReference(InputActionAsset asset, string action, string name)
        {
            ...
            AssetDatabase.CreateAsset(reference, Root + "/" + name + ".asset");
            return reference;
        }
