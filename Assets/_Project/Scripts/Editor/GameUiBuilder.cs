...
public static class GameUiBuilder
{
    ...
    public static string Build()
    {
        ...
        if(AssetDatabase.LoadAssetAtPath<GameObject>("Assets/_Project/Prefabs/UI/PF_MainMenuScreen.prefab")!=null)throw new Exception("UI already authored; edit prefabs instead of rebuilding.");
    ...
    {
        var path="Assets/_Project/Fonts/NanumGothic/NanumGothicSdf.asset";
        font=AssetDatabase.LoadAssetAtPath<TMP_FontAsset>(path);
        if(font!=null)return;
        font=TMP_FontAsset.CreateFontAsset(AssetDatabase.LoadAssetAtPath<Font>("Assets/_Project/Fonts/NanumGothic/NanumGothic-Regular.ttf"));
    ...
    {
        settings=ScriptableObject.CreateInstance<GameUiSettings>(); settings.menuScene="Assets/_Project/Scenes/01_Title/MainMenu.unity";
        AssetDatabase.CreateAsset(settings,"Assets/_Project/Data/UI/SO_GameUiSettings.asset");
        ...
        System.IO.File.WriteAllText("Assets/_Project/Settings/Input/GameUiControls.inputactions",asset.ToJson());
        Object.DestroyImmediate(asset);
        AssetDatabase.ImportAsset("Assets/_Project/Settings/Input/GameUiControls.inputactions",ImportAssetOptions.ForceSynchronousImport);
        var imported=AssetDatabase.LoadAssetAtPath<InputActionAsset>("Assets/_Project/Settings/Input/GameUiControls.inputactions");
        settings.pauseAction=Ref(imported.FindAction("Pause"),"Pause"); settings.helpAction=Ref(imported.FindAction("Help"),"Help");
        var player=AssetDatabase.LoadAssetAtPath<InputActionAsset>("Assets/_Project/Settings/Input/PlayerControls.inputactions");
        var stage=AssetDatabase.LoadAssetAtPath<InputActionAsset>("Assets/_Project/Settings/Input/StageControls.inputactions");
    ...
    private static InputActionReference Ref(InputAction action,string name)
    {
        ...
        var r=InputActionReference.Create(action); AssetDatabase.CreateAsset(r,"Assets/_Project/Settings/Input/References/UI/"+name+".asset");return r;
    }
    ...
    { var prefab=PrefabUtility.SaveAsPrefabAsset(g,"Assets/_Project/Prefabs/UI/PF_"+name+".prefab");Object.DestroyImmediate(g);return prefab; }
    ...
    private static void Background(Transform parent,string texture)
    {
        ...
        var raw=g.AddComponent<RawImage>();raw.texture=AssetDatabase.LoadAssetAtPath<Texture2D>("Assets/_Project/Art/UI/Textures/T_"+texture+".png");raw.raycastTarget=false;
    }
    ...
    {
        var root=CanvasRoot("MainMenuScreen",10);var ui=root.AddComponent<MainMenuScreen>();ui.settings=settings;
        ...
        return Save(root,"MainMenuScreen");
    }
    private static GameObject BuildOverlay()
    {
        var root=CanvasRoot("GameOverlayHud",30);var ui=root.AddComponent<GameOverlayHud>();ui.settings=settings;
        ...
        return Save(root,"GameOverlayHud");
    }
    private static GameObject BuildHud()
    {
        var root=Rect("PlayerHud",null,Vector2.zero,new Vector2(640,720));var ui=root.AddComponent<PlayerHud>();ui.settings=settings;
    ...
    {
        var scene=EditorSceneManager.OpenScene("Assets/_Project/Scenes/01_Title/MainMenu.unity");
    ...
    private static void ConfigurePlay(string path,string title,string objective)
    {
        ...
        for(int i=0;i<regions.arraySize;i++)
        {
            ...
            var hud=Instance(hudPrefab,parent);Stretch(hud);hud.GetComponent<PlayerHud>().slot=item.FindPropertyRelative("spawnSlot").intValue;
            PrefabUtility.RecordPrefabInstancePropertyModifications(hud.GetComponent<PlayerHud>());
        }
        ...
        var overlay=Instance(overlayPrefab,null).GetComponent<GameOverlayHud>();overlay.title=title;overlay.defaultObjective=objective;
