...
Start with Assets/_Project/Scenes/01_Title/MainMenu.unity (first enabled build scene).
...
4. At two active players, the host closes the room and calls Fusion LoadScene for Sandbox_Abilities. The client follows via NetworkSceneManagerDefault.
...
Sandbox_Abilities now uses PlayerSpawnLayout to disable the offline player during sessions and spawn two predicted NetworkPlayers. Movement, Space jump and body/head state are synchronized; see Assets/_Project/Documentation/Player/README.md.

## Configuration

Assets/_Project/Data/Networking/SO_LobbySettings.asset controls room size, scene paths, room-name limit, connection timeout, region, app version and status messages.
...
- Real two-process test passed: Unity Editor Host stayed in MainMenu at 1/2; a Windows Development Build joined via Photon; both peers reached Sandbox_Abilities with two active players.
