...
## Prefabs and scenes
- MainMenuUI: home, room-code entry, two-character selection, loading and options instances.
...
- GameOverlayUI: stage name/objective, subtitle hook, shared pause menu and options/loading.
- PlayerHudUI: per-viewport ability markers, help, held item, feedback and interaction prompt.
- Integrated into MainMenu, PlayerMovementPrototype, StageOnePrototype and StageTwoPrototype.
...
NanumGothic is included under SIL OFL; see Fonts/OFL.txt.
Stage One settings and item names are localized into Korean.
GameOverlayUI.ShowSubtitle is a hook for future dialogue/narration; no scripted dialogue timeline is fabricated.
The current options include volume and mouse sensitivity; further option content is not specified in the PDF.
See Assets/PlayerPrototype/PrototypeModeMenu.md for removal of the temporary solo entry.
