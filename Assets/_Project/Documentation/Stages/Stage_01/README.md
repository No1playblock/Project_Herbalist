...
- MainMenu multiplayer -> Stage_01_Exterior. Single Player -> original Sandbox_Abilities for independent ability testing.
- Stage_01_Exterior now contains the Sacred_Tree exterior. Two herb points lie on opposite sides of the tree; one requires a short rock jump sequence. ExpectedExteriorRoute is an Editor-only planning guide. SacredTree_Entrance/StageTwo_EntryVolume replaces the old freestanding test door. Both players entering after potion completion advances to Stage_02_Interior.
...
- onEntranceOpened and onCleared fire once on each peer for presentation. A future scene-transition listener must check StageOneFlow.Authority and use Fusion scene loading. StageExit now invokes the Host session transition to the configured Stage_02_Interior scene.
...
- Direct Editor StageOne play exposes Duyeong alone for visual/gather testing; it cannot bypass two-player progression. Use original Sandbox_Abilities for solo ability testing.
...
See Assets/_Project/Documentation/Stages/Stage_02/README.md for layout, ability persistence and transition verification. Historical verification below/above describes the earlier standalone stage test.
