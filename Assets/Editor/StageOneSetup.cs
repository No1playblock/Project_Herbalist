...
namespace Herbalist.Editor
{
    ...
    {
        public const string Folder = "Assets/StageOne";
        public const string ScenePath = Folder + "/StageOnePrototype.unity";
        public static string Create()
        {
            ...
            if (!AssetDatabase.IsValidFolder(Folder)) AssetDatabase.CreateFolder("Assets", "StageOne");
            ...
            AssetDatabase.CreateAsset(settings, Folder + "/StageOneSettings.asset");
            ...
            System.IO.File.WriteAllText(Folder + "/StageControls.inputactions", controls.ToJson());
            UnityEngine.Object.DestroyImmediate(controls); AssetDatabase.ImportAsset(Folder + "/StageControls.inputactions");
            controls = AssetDatabase.LoadAssetAtPath<InputActionAsset>(Folder + "/StageControls.inputactions");
            var actions = new InputActionReference[3];
            string[] actionNames = { "Interact", "Craft", "Drink" };
            for (int i=0;i<3;i++) { actions[i] = InputActionReference.Create(controls.FindAction(actionNames[i], true)); AssetDatabase.CreateAsset(actions[i], Folder + "/" + actionNames[i] + ".asset"); }
            var carryMaterial = new Material(AssetDatabase.LoadAssetAtPath<Material>("Assets/PlayerPrototype/Sap.mat"));
            carryMaterial.name = "StageCarry"; AssetDatabase.CreateAsset(carryMaterial, Folder + "/StageCarry.mat");
            var networkPrefab = PrefabUtility.LoadPrefabContents("Assets/PlayerPrototype/NetworkPlayer.prefab");
            ...
            var saved = PrefabUtility.SaveAsPrefabAsset(networkPrefab, Folder + "/StageNetworkPlayer.prefab");
            PrefabUtility.UnloadPrefabContents(networkPrefab);
            AssetDatabase.CopyAsset("Assets/PlayerPrototype/PlayerMovementPrototype.unity", ScenePath);
            ...
            var wood = AssetDatabase.LoadAssetAtPath<Material>("Assets/PlayerPrototype/Obstacles.mat");
            ...
            AssetDatabase.ImportAsset(Folder + "/StageNetworkPlayer.prefab", ImportAssetOptions.ForceUpdate);
        ...
        private static StageItemDefinition Item(string name, string label, StageItemType type, PlayerAbilityKind ability, Color color)
        {
            ...
            AssetDatabase.CreateAsset(item, Folder + "/" + name + ".asset"); return item;
        }
        private static StageRecipe Recipe(string name, StageItemDefinition herb, StageItemDefinition potion)
        { var r = ScriptableObject.CreateInstance<StageRecipe>(); r.ingredient = herb; r.result = potion; AssetDatabase.CreateAsset(r, Folder + "/" + name + ".asset"); return r; }
        ...
        private static HerbSource Source(Transform parent,string name,Vector3 position,StageItemDefinition item,int glowLayer)
        {
            ...
            var mat=new Material(Shader.Find("Universal Render Pipeline/Lit"));mat.color=item.color; AssetDatabase.CreateAsset(mat,Folder+"/"+name+".mat");
            source.visual=Primitive("Herb",go.transform,Vector3.zero,new Vector3(.3f,.7f,.3f),mat,false);
            var glowMaterial=new Material(Shader.Find("Universal Render Pipeline/Unlit"));glowMaterial.color=item.color*1.7f; AssetDatabase.CreateAsset(glowMaterial,Folder+"/"+name+"_Glow.mat");
