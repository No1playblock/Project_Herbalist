...
namespace Herbalist.Editor
{
    public static class SapAbilitySetup
    {
        ...
        public static string Apply()
        {
            ...
            var settings = AssetDatabase.LoadAssetAtPath<SapAbilitySettings>(Folder + "SapAbilitySettings.asset");
            if (settings == null) { settings = ScriptableObject.CreateInstance<SapAbilitySettings>(); AssetDatabase.CreateAsset(settings, Folder + "SapAbilitySettings.asset"); }
            var material = AssetDatabase.LoadAssetAtPath<Material>(Folder + "Sap.mat");
            if (material == null)
            {
                ...
                AssetDatabase.CreateAsset(material, Folder + "Sap.mat");
            }
            var offline = AssetDatabase.LoadAssetAtPath<GameObject>(Folder + "SapDeposit.prefab");
            var network = AssetDatabase.LoadAssetAtPath<GameObject>(Folder + "NetworkSapDeposit.prefab");
            if (offline == null || network == null)
            {
                ...
                offline = PrefabUtility.SaveAsPrefabAsset(root, Folder + "SapDeposit.prefab");
                root.AddComponent<NetworkObject>(); root.AddComponent<NetworkTransform>(); root.AddComponent<NetworkSapDeposit>();
                network = PrefabUtility.SaveAsPrefabAsset(root, Folder + "NetworkSapDeposit.prefab");
            ...
            foreach (var name in new[] { "Player.prefab", "NetworkPlayer.prefab" })
            {
                ...
                try
                {
                    ...
                    PrefabUtility.SaveAsPrefabAsset(root, Folder + name);
                }
            ...
            if (!scene.GetRootGameObjects().Any(g => g.name == "SapAbilityTestObjects"))
            {
                ...
                tree.GetComponent<Renderer>().sharedMaterial = AssetDatabase.LoadAssetAtPath<Material>(Folder + "Obstacles.mat");
