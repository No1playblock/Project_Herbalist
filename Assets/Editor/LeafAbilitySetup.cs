...
namespace Herbalist.Editor
{
    ...
    {
        private const string Folder = "Assets/PlayerPrototype/";
        public static string Apply()
        {
            ...
            var settings = AssetDatabase.LoadAssetAtPath<LeafAbilitySettings>(Folder + "LeafAbilitySettings.asset");
            if (settings == null) { settings = ScriptableObject.CreateInstance<LeafAbilitySettings>(); AssetDatabase.CreateAsset(settings, Folder + "LeafAbilitySettings.asset"); }
            var mat = AssetDatabase.LoadAssetAtPath<Material>(Folder + "Leaf.mat");
            if (mat == null) { mat = new Material(AssetDatabase.LoadAssetAtPath<Material>(Folder + "Body.mat")); mat.color = new Color(0.3f,0.7f,0.15f); AssetDatabase.CreateAsset(mat, Folder + "Leaf.mat"); }
            ...
            var offlinePrefab = PrefabUtility.SaveAsPrefabAsset(leaf, Folder + "LeafProjectile.prefab");
            ...
            var netPrefab = PrefabUtility.SaveAsPrefabAsset(leaf, Folder + "NetworkLeafProjectile.prefab");
            Object.DestroyImmediate(leaf);
            foreach (string name in new[] { "Player.prefab", "NetworkPlayer.prefab" })
            {
                var root = PrefabUtility.LoadPrefabContents(Folder + name);
                ...
                PrefabUtility.SaveAsPrefabAsset(root,Folder+name); PrefabUtility.UnloadPrefabContents(root);
            }
            var scene = SceneManager.GetSceneByPath(Folder + "PlayerMovementPrototype.unity");
            bool opened = !scene.isLoaded;
            if (opened) scene = EditorSceneManager.OpenScene(Folder+"PlayerMovementPrototype.unity",OpenSceneMode.Additive);
            ...
            if (!scene.GetRootGameObjects().Any(g=>g.name=="LeafAbilityTestTargets"))
            {
                ...
                for(int i=0;i<2;i++)
                {
                    ...
                    board.GetComponent<Renderer>().sharedMaterial=AssetDatabase.LoadAssetAtPath<Material>(Folder+"Obstacles.mat");
        ...
        {
            var reference=AssetDatabase.LoadAssetAtPath<InputActionReference>(Folder+name);
            if(reference==null){reference=InputActionReference.Create(action);AssetDatabase.CreateAsset(reference,Folder+name);}return reference;
        }
