...
namespace Herbalist.Abilities
{
    ...
    public sealed class SapStreamPresentation : MonoBehaviour
    {
        ...
        {
            if(!Herbalist.GameUI.GameplayPause.IsPaused)flowTime+=Time.deltaTime;
            ...
            for (int i = 0; i < beads.Length; i++)
            {
                ...
                float t = Mathf.Repeat(flowTime * settings.streamFlowSpeed / Mathf.Max(length, settings.radius) + (float)i / beads.Length, 1);
