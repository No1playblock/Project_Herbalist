...
Open `Assets/_Project/Scenes/Development/Sandbox_Abilities.unity` for offline testing. The offline player receives Sap. MainMenu still connects two players and opens this scene; prototype slot 0 (Duyeong) receives Sap and slot 1 (Sodam) receives Leaf. This is an Inspector-configured test assignment, not a character restriction. Potion code should call the authoritative `UnlockSap` / `UnlockLeaf` hooks. A player cannot acquire both simultaneously.
